// ErrorMsg.h — Phase 7.5 server-side stub for [Server]Map.
//
...
//
// Instead, this stub sits in [Server]Map/ which is already on the
// include path via target_include_directories. Because CMake passes
// the target's include dirs before transitive deps, this stub wins
// the #include "ErrorMsg.h" lookup before any other path.
//
...
//
// This stub mirrors [Server]Distribute/ErrorMsg.h from Phase 7.4a
// (commit 99c1559 + Bug C-27). It exists in [Server]Map/ rather than
// a shared location because the CMake build for Map is configured
// independently and the [Server]Map include dir comes first in
// target_include_directories, so the stub wins the lookup.
...
// (grep verified across [Server]Map / [CC]ServerModule), so we leave
// it undefined. If a future server file does use it, the compile
// error will point here and we'll know to wire it up properly.
//
// Uncomment if needed:
