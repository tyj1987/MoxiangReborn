...
{
	//////////////////////////////////////////////////////////////////////////
	// ?횙쩍횄 쨋짬쨩짠 //쉥된섬掘齡99돨밍膠====侶쟁慨빎唐狂痙,닸虜
...
#ifndef _JAPAN_LOCAL_	//일본의 경우는 크리티컬을 2.25배가 아닌 1.5배로 정상적으로 수정된다.
	if(bCritical)
	{
...
//		double dS = SimMek;
		//쨀쨩째첩 째첩째횦쨌횂 = [ 쨀쨩째첩 째첩째횦쨌횂 * {(쩍횋쨍횈 +200)/200}*{(쩍횋쨍횈+1000)/500}+쩍횋쨍횈]*0.6
...
double	CAttackCalc::getPhyDefenceLevel(CObject* pObject, CObject* pAttacker)
{
	...
	if( pObject->GetObjectKind() == eObjectKind_Player )
	{
		if( ((CPlayer*)pObject)->InTitan() )
		{
			...
			WORD shopItemTitanDefense = 0;	//추가 작업
