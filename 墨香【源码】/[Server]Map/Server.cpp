                     ...
                     int       nCmdShow)
{
	...
	if( CheckUpdateFile() == FALSE )
	{
		...
		g_Console.MLOG(LOG_IMPORTANT, "CheckUpdateFile failed, exiting MapServer.");
...
BOOL CheckUpdateFile()
{
	...
	{
		g_Console.MLOG(LOG_IMPORTANT, "CheckUpdateFile failed: TitanServer.bin Init() returned FALSE (file missing or header/CRC bad).");
	...
	{
		g_Console.MLOG(LOG_IMPORTANT, "CheckUpdateFile failed: decoded TitanServer.bin sentinel mismatch (got '%s').", temp);
