...
// Phase 7.2: legacy #pragma referenced a file (SS3DGFuncN.lib) that
// doesn't ship in this source tree. The CMaked build copies
// SS3DGFunc.lib (the SS3DGFunc.dll import library) next to the
// build output and links it as a bare filename, so this #pragma
// must point at the same file. SS3DGFuncN.dll on disk is the same
// version as SS3DGFunc.dll, just with a "Native CRT" suffix in
// the old legacy naming scheme; the .lib side is interchangeable.
