...
//	if(!file.Init("./Resource/ItemChangeRateofLv.bin", "rb"))
	...
	{
		// Phase 7.5 fix: original line had a Korean MessageBox caption
		// (`ItemChangeRateofLv.bin - ������...` + `���`) which the MBCS
		// source-encoding pipeline mangles into invalid byte sequences
		// (C2001 newline in constant). Replaced with ASCII so the
		// compile succeeds; runtime message is functionally equivalent.
		::MessageBox(NULL,"ItemChangeRateofLv.bin - load failed","ALERT",MB_OK);
		return;
	}
#else
