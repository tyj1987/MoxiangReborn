...
- The canonical `playdh-current/Resource/Server/MonsterDropItemList.bin` and the bundled `-922`/`-9-30` variants are byte-readable but all decode to the `$Group` AIGroup grammar, not the legacy 10-entry drop-table grammar. The new loader fails closed and the live combat probe therefore receives no `MonsterObtainNotify`; the correct drop-table variant/profile must be recovered before loot/pickup can be marked playable.
