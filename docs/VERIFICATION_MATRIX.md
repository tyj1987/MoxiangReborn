...
| `EVID-20260825-current-map10-blocked` | `cd06a441` | `playdh-current` (`server-size-prefixed-opaque-v1`) | `start_modern.ps1 -Mode start -ResourceProfileId playdh-current -MapNumber 10` | correctly fails closed: current 22,766-byte `Monster_10.bin` variant is rejected by the AIGroup decoder; no reference-profile substitution occurs |
| `EVID-20260825-full-ctest` | `26fbea90` | local Debug build | `ctest -C Debug --test-dir modern/build --output-on-failure` | 12,133 registered; 12,133 passed; 19 tests skip for explicit missing/optional fixtures. BGM playback uses Media Foundation/XAudio2 and login secret/recovery tests are included; this is still not an RC gate because the skipped runtime/resource gates and human path remain open. |
