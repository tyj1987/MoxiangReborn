...
| `EVID-20260825-static-collision-query` | `568c2d55` | STM bounds + in-game movement fixture | `cmd /c scripts\\build-modern.bat` + `modern/build/tests/unit/client/mxh_client_tests.exe --gtest_filter=InGamePlayable.StaticCollisionQueryRejectsCandidateStep --gtest_brief=1` | loaded STM mesh bounds are exposed through an explicit collision query and blocked candidate movement is rejected. |
