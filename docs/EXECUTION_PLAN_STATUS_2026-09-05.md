...
**mxh_client_tests 基线**: 319/319 PASSED in 16.5 sec (从 299 → 319, +20 新 test)
...
- **Phase 1 §7.2 登录错误矩阵 14 项** — **本 session 已加 11/14**: 5 boundary (17B 截断,中文 UTF-8,空字段,boundary,invalid port) + 4 Nack (LoginNackTriggersFailWith, LoginNackIsIdempotent, LoginAckAfterNackDoesNotRecover, FailWithIsIdempotent) + 2 disconnect (DisconnectBeforeAckTriggersFailWith, DisconnectAfterAckIsIgnored). 剩 3 项 (重复注册, 登录超时, AgentServer down) 需 real server
