...
TEST(InventoryExDialog, AddsQueriesAndCountsSlots){cInventoryExDialog d;EXPECT_EQ(d.GetBlankNum(),80u);EXPECT_TRUE(d.AddItem(100,20));EXPECT_TRUE(d.IsExist(0));EXPECT_EQ(d.GetItemForPos(0)->item_id,100);EXPECT_EQ(d.GetBlankNum(),79u);}
...
TEST(InventoryExDialog, RefreshesTheLastWireSlot){cInventoryExDialog d;InventorySnapshot snapshot;snapshot.items[79]=mxh::game::make_item(43,654,79,88);d.SetInventoryService(&snapshot);d.RefreshFromInventoryService();ASSERT_TRUE(d.GetItemForPos(79).has_value());EXPECT_EQ(d.GetItemForPos(79)->item_id,654);EXPECT_EQ(d.GetItemForPos(79)->durability,88);}
