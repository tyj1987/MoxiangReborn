...
using mxh::ui::cMNCreateDialog;
using mxh::ui::cStatic;

TEST(MNCreateDialogTest, InheritsDialogAndIsNonCopyable) {
    static_assert(std::is_base_of_v<cDialog, cMNCreateDialog>);
    static_assert(!std::is_copy_constructible_v<cMNCreateDialog>);
    static_assert(!std::is_copy_assignable_v<cMNCreateDialog>);
    static_assert(std::has_virtual_destructor_v<cMNCreateDialog>);
    SUCCEED();
}

TEST(MNCreateDialogTest, ConstructorKeepsDialogDefaults) {
    cMNCreateDialog dialog;
...
TEST(MNCreateDialogTest, LinkingIsEmptyAndPreservesGeometry) {
    cMNCreateDialog dialog;
...
TEST(MNCreateDialogTest, LinkingPreservesActiveState) {
    cMNCreateDialog dialog;
...
TEST(MNCreateDialogTest, LinkingPreservesDisableState) {
    cMNCreateDialog dialog;
...
TEST(MNCreateDialogTest, LinkingPreservesOwnedChildren) {
    cMNCreateDialog dialog;
...
TEST(MNCreateDialogTest, ActionEventIsEmptyForZeroInput) {
    cMNCreateDialog dialog;
...
TEST(MNCreateDialogTest, ActionEventIgnoresAllLegacyArguments) {
    cMNCreateDialog dialog;
    int payload = 42;
...
TEST(MNCreateDialogTest, ActionEventPreservesActiveAndDisableState) {
    cMNCreateDialog dialog;
    dialog.SetActive(true);
    dialog.SetDisable(true);
...
TEST(MNCreateDialogTest, ActionEventPreservesGeometryAndIdentity) {
    cMNCreateDialog dialog;
...
TEST(MNCreateDialogTest, RepeatedLegacyCallsRemainIdempotent) {
    cMNCreateDialog dialog;
...
TEST(MNCreateDialogTest, DestructionThroughDialogBaseIsSafe) {
    std::unique_ptr<cDialog> dialog = std::make_unique<cMNCreateDialog>();
    dialog->SetActive(true);
    EXPECT_TRUE(dialog->isActive());
}
