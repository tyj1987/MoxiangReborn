...
TEST(FriendDialog, LinkingBuildsLiveListRows) {
  cFriendDialog dialog;
  auto list = std::make_unique<mxh::ui::cListCtrl>();
  list->Init(0, 0, 103, 160, nullptr, 0);
  list->setLegacyId("FRI_FRIENDLISTLCTL");
  dialog.Add(std::move(list));
  dialog.Linking();
  dialog.AddFriend({7, "Alice", FriendStatus::Online});
  dialog.AddFriend({8, "Bob", FriendStatus::Offline});
  dialog.RefreshFriendList();
  ASSERT_NE(dialog.FriendList(), nullptr);
  ASSERT_EQ(dialog.FriendList()->rowCount(), 2u);
  EXPECT_NE(dialog.FriendList()->rowAt(0).texts[0].find("Alice"), std::string::npos);
  EXPECT_NE(dialog.FriendList()->rowAt(0).texts[0].find("Online"), std::string::npos);
  EXPECT_NE(dialog.FriendList()->rowAt(1).texts[0].find("Offline"), std::string::npos);
}
