...
TEST(CTitanMixProgressBarDlgTest, OnActionEventCancelInvokesHostReEnable) {
    LinkedDialog ld;
    int calls = 0;
...
TEST(CTitanMixProgressBarDlgTest, OnActionEventCancelWithoutCallbackIsSafe) {
    LinkedDialog ld;
    ld.dlg.StartProgress();
    EXPECT_TRUE(ld.dlg.IsProgressStart());
    ld.dlg.OnActionEvent(cTitanMixProgressBarDlg::kIdCancelBtn, nullptr, 0);
...
TEST(CTitanMixProgressBarDlgTest, OnActionEventNonCancelDoesNotInvokeReEnable) {
    LinkedDialog ld;
    int calls = 0;
    auto cb = [](void* userData) {
        ++*static_cast<int*>(userData);
...
TEST(CTitanMixProgressBarDlgTest, OnActionEventCancelReplacesCallback) {
    LinkedDialog ld;
    int firstCalls = 0;
    int secondCalls = 0;
...
TEST(CTitanMixProgressBarDlgTest, OnActionEventCancelWithNullUserDataIsSafe) {
    LinkedDialog ld;
    auto cb = [](void*) {};
