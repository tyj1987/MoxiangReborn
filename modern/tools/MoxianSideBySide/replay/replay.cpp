...
//
// Format contracts (locked against modern LoginHandler/AgentHandler):
//   login      -> LoginServer port  payload >=38B
//                 [AuthKey: u32][id: 17B][pw: 17B]
//   enter_game -> AgentServer port  two-step:
//                 proto=16  object_id=char_id payload=[channel:u16]
//                 proto=28  object_id=char_id payload=[channel:u32]
//   attack     -> MapServer port    proto=4  payload=[skill:u16][target:u32]
//   shop       -> MapServer port    proto=17 payload=[item:u16][qty:u16]
//   quest      -> MapServer port    proto=1  payload=[quest:u16]
#include "replay.hpp"
#include <cstring>
...
ReplayScenario attack_scenario() {
    ...
    put_u16(p.data(), 1);
    put_u32(p.data() + 2, 2);
    s.client_packets.push_back(mk(9, 4, 0, std::move(p)));
    return s;
}
...
ReplayScenario shop_scenario() {
    ...
    put_u16(p.data(), 1);
    put_u16(p.data() + 2, 1);
    s.client_packets.push_back(mk(5, 17, 0, std::move(p)));
    return s;
}
