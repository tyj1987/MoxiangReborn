"""Phase 7.1: Configure + Build YHLibrary via CMake (no shell)."""
import os
import subprocess
import sys
...
# Use the same vcvars64 setup that the Phase 7.0 POC uses.
def run_via_cmd(line) -> tuple[int, str, str]:
    ...
    for line2 in setup_out.splitlines():
        ...
        try:
            ...
            pass
    r = subprocess.run(line, capture_output=True, shell=False, env=env)
...
print()
print("=== Build ===")
rc, out, err = run_via_cmd([cmake_exe, "--build", BD])
print(out)
if rc != 0:
    print("STDERR:", err)
    sys.exit(rc)

# Check that YHLibrary.lib was produced.
print()
produced = None
for cand in (os.path.join(BD, "Release", "YHLibrary.lib"), os.path.join(BD, "YHLibrary.lib")):
