"""
Phase 7.5b: convert EUC-KR / cp949 bytes in [Server]Distribute sources to UTF-8.

Background: Phase 7.5b transcoded the legacy source tree from a mix of cp949
(Korean) / cp936 (Chinese) / cp1252 to UTF-8. The transcoding was applied to
[Server]Map (per the CMakeLists comment block in that tree) but [Server]Distribute
sources are still partially cp949 — when MSVC reads them with /source-charset:utf-8
the cp949 lead bytes don't decode as valid UTF-8 and the compiler dies with
"newline in constant" / "undeclared identifier" errors that cascade.

The cleanest fix: re-decode each affected .cpp as cp949 (which succeeds for the
cp949 byte sequences) and re-encode as UTF-8. The files are otherwise ASCII +
the cp949 Hangul sequences — no structural changes.

Idempotent: re-running on a file that's already UTF-8 should be a no-op.
We detect "already UTF-8" by trying UTF-8 decode first; if it succeeds without
substitution markers the file is left alone.
"""
...
ROOT = Path(r"D:\墨香全套源代码（源码+资源+客户端+服务端+教程）\墨香【源码】\[Server]Distribute")
...
def convert_one(path: Path) -> tuple[bool, str]:
    ...
    return True, f"converted ({raw!r} -> {new_raw!r})"
...
def main(argv: list[str] | None = None) -> int:
    ...
    root = Path(args.root)
    targets = sorted(p for p in root.glob("*.cpp") if p.is_file())
    if not targets:
        print(f"no .cpp files under {root}", file=sys.stderr)
        return 1

    print(f"=== Phase 7.5b: cp949 -> utf-8 for {len(targets)} files in {root} ===")
    if args.dry_run:
