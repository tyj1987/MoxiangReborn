//
// D4 -- AgentTitan side-effect plan.
//
...
// MP_TITAN traffic arrives. The default branch is:
//
...
enum class AgentTitanSideEffectKind : std::uint8_t {
    Drop,
    ForwardToUser,
};

struct AgentTitanSideEffect final {
    AgentTitanSideEffectKind kind = AgentTitanSideEffectKind::Drop;
    std::uint8_t reply_protocol = 0u;
    std::uint32_t connection_index = 0u;
    std::uint32_t object_id = 0u;
};

struct AgentTitanSideEffectPlan final {
    std::vector<AgentTitanSideEffect> effects;
    bool dispatched = false;
    bool drop = true;
};

inline bool titan_effect_targets_user(const AgentTitanSideEffect& e) noexcept {
    return e.kind == AgentTitanSideEffectKind::ForwardToUser;
}

inline AgentTitanSideEffectPlan agent_titan_side_effect_plan(
    const AgentTitanRequest& r,
    std::uint32_t connection_index) {
    AgentTitanSideEffectPlan plan;
    using K = AgentTitanSideEffectKind;
    using O = AgentTitanOutcome;
    const auto outcome = classify_agent_titan(r);
    switch (outcome) {
