...
struct WantedRequest { std::uint8_t protocol=0; std::uint32_t object_id=0; bool user_found=true; std::uint8_t target_map_connection_index=0; };
struct WantedAction { WantedServerActionKind kind=WantedServerActionKind::default_forward_to_client; std::uint8_t protocol=0; std::uint32_t object_id=0; std::uint8_t target_map_connection_index=0; };
