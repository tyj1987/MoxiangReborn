...
struct MurimNetPlayerInfo { std::uint32_t player_id=0; std::uint32_t agent_num=0; std::uint32_t unique_id_in_agent=0; std::uint32_t back_map_num=0; std::string name; std::uint16_t level=0; std::uint16_t rank_point=0; std::string nick; std::uint16_t play_count=0; std::uint16_t win=0; std::uint16_t lose=0; std::string munpa; std::int8_t position_in_munpa=0; MnTeam team=MnTeam::Left; };
