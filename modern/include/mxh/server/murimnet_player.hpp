...
class MurimNetPlayer final { public: bool init(const MurimNetPlayerInfo&) noexcept; const MurimNetPlayerInfo& info() const noexcept{return m_info;} std::uint32_t id() const noexcept{return m_info.player_id;} MnRoomPlayer& room_state() noexcept{return m_roomState;} const MnRoomPlayer& room_state() const noexcept{return m_roomState;} private: MurimNetPlayerInfo m_info{}; MnRoomPlayer m_roomState{}; };
