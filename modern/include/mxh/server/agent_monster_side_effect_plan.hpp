//
// D4 -- AgentMonster side-effect plan.
//
...
// MP_MONSTER traffic arrives. The default branch is:
//
...
enum class AgentMonsterSideEffectKind : std::uint8_t {
    Drop,
    ForwardToUser,
};

struct AgentMonsterSideEffect final {
    AgentMonsterSideEffectKind kind = AgentMonsterSideEffectKind::Drop;
    std::uint8_t reply_protocol = 0u;
    std::uint32_t connection_index = 0u;
    std::uint32_t object_id = 0u;
};

struct AgentMonsterSideEffectPlan final {
    std::vector<AgentMonsterSideEffect> effects;
    bool dispatched = false;
    bool drop = true;
};

inline bool monster_effect_targets_user(const AgentMonsterSideEffect& e) noexcept {
    return e.kind == AgentMonsterSideEffectKind::ForwardToUser;
}

inline AgentMonsterSideEffectPlan agent_monster_side_effect_plan(
    const AgentMonsterRequest& r,
    std::uint32_t connection_index) {
    AgentMonsterSideEffectPlan plan;
    using K = AgentMonsterSideEffectKind;
    using O = AgentMonsterOutcome;
    const auto outcome = classify_agent_monster(r);
    switch (outcome) {
