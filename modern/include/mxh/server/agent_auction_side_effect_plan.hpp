//
// D4 -- AgentAuction side-effect plan.
//
...
// MP_MP_AUCTION traffic arrives. The default branch is:
//
...
enum class AgentAuctionSideEffectKind : std::uint8_t {
    Drop,
    ForwardToUser,
};

struct AgentAuctionSideEffect final {
    AgentAuctionSideEffectKind kind = AgentAuctionSideEffectKind::Drop;
    std::uint8_t reply_protocol = 0u;
    std::uint32_t connection_index = 0u;
    std::uint32_t object_id = 0u;
};

struct AgentAuctionSideEffectPlan final {
    std::vector<AgentAuctionSideEffect> effects;
    bool dispatched = false;
    bool drop = true;
};

inline bool mp_auction_effect_targets_user(const AgentAuctionSideEffect& e) noexcept {
    return e.kind == AgentAuctionSideEffectKind::ForwardToUser;
}

inline AgentAuctionSideEffectPlan agent_auction_side_effect_plan(
    const AgentAuctionRequest& r,
    std::uint32_t connection_index) {
    AgentAuctionSideEffectPlan plan;
    using K = AgentAuctionSideEffectKind;
    using O = AgentAuctionOutcome;
    const auto outcome = classify_agent_auction(r);
    switch (outcome) {
