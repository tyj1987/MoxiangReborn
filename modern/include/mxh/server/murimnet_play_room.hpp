...
class MurimNetPlayRoom final { public: bool create(const MnPlayRoomCreateInfo&); void release() noexcept; bool player_in(MnRoomPlayer&); bool player_out(MnRoomPlayer&); bool team_change(MnRoomPlayer&,MnTeam,MnTeam); void play_start(bool) noexcept; bool started() const noexcept{return m_started;} std::size_t player_count() const noexcept{return m_players.size();} std::size_t team_count(MnTeam) const noexcept; bool contains(std::uint32_t) const noexcept; std::uint32_t room_index() const noexcept{return m_info.room_index;} const std::string& title() const noexcept{return m_info.title;} std::uint8_t kind() const noexcept{return m_info.kind;} std::uint16_t max_observers() const noexcept{return m_info.max_observers;} std::uint16_t max_players_per_team() const noexcept{return m_info.max_players_per_team;} std::uint32_t money_for_play() const noexcept{return m_info.money_for_play;} std::uint16_t map_num() const noexcept{return m_info.map_num;} void for_each_member(const std::function<void(const MnRoomPlayer&)>&) const; private: MnPlayRoomCreateInfo m_info{}; std::vector<MnRoomPlayer*> m_players; std::array<std::vector<MnRoomPlayer*>,3> m_teams; bool m_started=false; };
}
