...
| `[Server]Map` §           | **0 errors** (7.5c Debug_KOR + 7.5d Release, build artifact reproducible, smoke criterion env-limited) | VS17 / x86 | `modern/scripts/build_map.py` (Debug_KOR) + `modern/scripts/build_map_release.py` (Release) | `MapServer_KOR.exe` 3,852,288 bytes (Debug) / `MapServer.exe` 1,283,584 bytes (Release) | **2026-07-08 7.5c producer wipe+rebuild x2** (Debug_KOR): `errors: 0 / warnings: 75,663 / rc: 0` per `build_map_full.txt`. EXE byte-count reproducible across two independent runs (SHA-256 `B5DDBD49…91119F`); `+50.7%` vs SWorking baseline 2,555,904. **2026-07-08 7.5d producer wipe+rebuild x1** (Release, after Bug C-30 fix in commit `befc5c1`): `errors: 0 / warnings: 75,680 / rc: 0` per `build_map_release_full.txt`. EXE `Release/MapServer.exe` = 1,283,584 bytes; `-49.8%` vs SWorking. **Both byte deltas are inside the proposed ±60% byte-parity threshold** (structural toolchain drift; not regressions). Full rationale: `modern/docs/phase7.5d_release_diagnostic.md` §3. The 7 defensive legacy-file fixes committed in `2f8b648` resolved the cascade the prior planner reported. **Phase 7.5d gate outcome** (`plan_cc0c5b57`, override_accept applied 2026-07-08 by orchestrator): independent wipe+rebuild ×2 reproduced `Release/MapServer.exe` at 1,283,584 bytes (2/2 byte-count reproducible; SHA-256 non-deterministic, same caveat Phase 7.5c used for Debug_KOR); dumpbin /dependents clean of renderer DLLs (SS3DGFunc.dll + KERNEL32 + USER32 + ole32 + WINMM only); byte parity -49.78% within ±60% band with structural-rationale attribution (MSVC 14.44 /O2 + /GF /Gy + AVX2 + no-debug-info codegen shift vs VS2008 /O2); exact git scope match (2 files in `befc5c1` + 3 files in `9a9f41f`, both exactly as expected); all 4 anti-fraud sub-checks OK (errors-before/after/byte-count/reproducibility markers in commit body; negation-context-only substrings in docs; build log 0 errors / 46 MB readable; diagnostic.md has 6 required sections + 1 extra). Smoke criterion (Check 4) not satisfied in this env — process alive @10s but 0 listening TCP ports; control experiment with legacy VS2008 `SWorking/MapServer.exe` (2,555,904 bytes) in same 6-file minimal staging died within 1s, confirming smoke failure is environmental (no SQL Server / MonitoringServer / full `ServerSet\1\` / `Resource\`), not a build regression. dumpbin dep list missing `WS2_32.dll` / `ADVAPI32.dll` / `ODBC32.dll` / `4DyuchiNET.dll` imports flagged as a real follow-up concern (consistent with Phase 7.5c Debug_KOR finding) — recommended separate diagnostic plan to root-cause whether this is delay-loading, `_KOR_LOCAL_` code-path gating, or a real link-spec bug. Producer has NOT written any "passed" / "verified" / "VERDICT" / "Gate 验证" / "Gate ✅" / "Gate PASS" narrative in either commit; only "fix applied", "0 errors", "EXE produced", "rebuild reproducible", "control confirms env-limit" per Bug E-1 anti-fraud discipline. |
...
| `[Server]MurimNet` ¶       | **RECIPE DONE** (7.6) | VS17 / x86  | `modern/scripts/build_murimnet.py`                    | *(4 locale errors prevent link)*                       | Bug D-8: 4 compile errors from locale-conditional fields (GetMapNum/MunpaName in HERO_TOTALINFO, CDataBase::Init overload). MurimNet is a Map-server variant (defines _MAPSERVER_ + _MURIMNET_) for PvP. 27 cpp (19 MurimNet + 7 ServerModule + 1 Header). CommonCalcFunc/CommonGameFunc/GameResourceManager excluded — need Map-specific headers. |
| `[Monitoring]Server`       | **SKIPPED**    | n/a              | n/a                                                  | n/a                                                   | Win32 GUI application (HWND + resource.h), not a console service. Different architecture from Agent/Distribute. |
...
- **C-30: Map's legacy Release config (Phase 7.5)** — `MSG_CHANNEL_INFO`
  ...
  built from Debug_Console (KOR). **Phase 7.5d fix (commit `befc5c1`,
  2026-07-08):** 5 access points wrapped with `#ifdef _KOR_LOCAL_ ...
  #endif`. Release now 0 errors, outputs `build_map/Release/MapServer.exe`
  = 1,283,584 bytes.
