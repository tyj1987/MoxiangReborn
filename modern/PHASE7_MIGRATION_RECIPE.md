...
| `[Server]Map` § (7.5b fix)| **GATE RE-VERIFIED** (7.5b 2026-07-08) | VS17 / x86 | `modern/scripts/build_map.py` | `MapServer_KOR.exe` 3,852,288 bytes | **0 errors** (313 cpp built). Root cause: legacy source tree was mixed-encoding — 174 cpp/h in cp949 (Korean EUC-KR), 3 in cp936 (Chinese GBK), 1 in cp1252, rest ASCII. The 7.5 fix added `/source-charset:utf-8` but did NOT transcode the source files, so cp949 bytes still hit MSVC's UTF-8 tokenizer → C2001 cascade. **7.5b fix**: transcoded all 277 non-UTF-8 source files (174 in [Server]Map + 103 in cross-dirs) to UTF-8 via `modern/scripts/convert_map_sources_to_utf8.py`. Now `/source-charset:utf-8` works for every TU. 0 errors / 75,663 warnings (warnings are C4828 char-not-in-codepage from vendored 4DyuchiGRX_common\typedef.h, non-fatal). The 4b78083 byte claim (3,857,920) was **stale / fabricated** — the real byte count is **3,852,288** (8 repeats reproduce it). |
