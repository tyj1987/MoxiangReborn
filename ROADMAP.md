...
| GM/运营工具 | GM API 强制认证；玩家、封禁、聊天、物品查询均接权威数据；活动持久化管理；物品发放使用幂等持久化投递箱，MapServer 进图时验证权威物品与背包容量后条件领取，失败保留待领取 | 在线领取后的完整角色物品 DB 快照仍需统一持久化；游戏服消费活动倍率仍需接线 | **运营控制面与可恢复物品投递闭环完成** |
