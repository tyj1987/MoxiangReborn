...
- [x] **C-Batch-2.41** cMPMissionDialog + cWantRegistDialog + cShoutchatDialog -- wired the legacy gCurTime TODOs through OPTIONAL host clock provider (same pattern as cFWEngraveDialog/cFWTimeDialog/cProgressBarDlg/cObjectGuagen). cMPMissionDialog SetActive(true) stamps m_dwStartTime via the host clock; ActionEvent auto-closes the dialog when curTime - startTime >= 5000ms (DWORD wrap-around preserved, legacy 5 sec gate). cWantRegistDialog SetActive(true) stamps m_dwStartShowTime + resets m_bShow; ActionEvent flips m_bShow when curTime - startShowTime >= 3000ms (legacy 3 sec gate); SetActive(false) also resets m_dwStartShowTime. cShoutchatDialog Process now throttles (early return when curTime - m_LastMsgTime < 5000ms) and updates m_LastMsgTime when the throttle window passes (legacy 5 sec gate). 21 new tests (9 MP + 7 WR + 5 SC) cover provider stamping, default + injected chat formatting analog, action-event auto-close timing, throttling, DWORD wrap-around, null-provider defensive paths, and reset behavior. Total 80 tests across the 3 dialogs.
