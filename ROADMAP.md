...
> **最近一次状态刷新**：2026-07-31 — Phase C Batch 2.3 完成（cChinaAdviceDlg，+26 tests，UI 套件 2243 全过）；Batch 1 全部完成（10/10 dialog），Batch 2.1+2.2+2.3 合计 3 个 dialog（cMPGuageDialog + cAlertDlg + cChinaAdviceDlg）。
...
| UI 控件 1:1 port（dialog + subcontrol） | **111/158 hpp（73 missing，Phase C Batch 1.10 完成 10/10 + Batch 2.1+2.2+2.3 完成 3 个）** | 老 client 158 dialog / modern 109 hpp（含 subcontrol+core）；Batch 1 已完成 cNumberPadDialog / cPartyWarDialog / cWantedDialog / cMPNoticeDialog / cReviveDialog / cMiniFriendDialog / cGuildInviteDialog / cChatOptionDialog / cStallKindSelectDlg / cDebugDlg / cMPGuageDialog / cMiniNoteDialog / cNoteDialog / cAutoAnswerDlg / cStatusIconDlg / cScreenShotDlg / cOptionDialog / cChannelDialog / cJournalDialog / cHelperSpeechDlg / cCostumeSkinSelectDialog（21 个，331 tests）；Batch 2.1+2.2+2.3 完成 cMPGuageDialog + cAlertDlg + cChinaAdviceDlg（3 个，95 tests）；剩 72 个老 dialog 待 1:1 port | T3 1:1 行为锁死 |
