...
| **M3 进展 (2026-08-10)** | T3 五段 modern 5/5 diff=0；现代金色锁像锁定在 modern/tests/fixtures/sbs_captures_modern/ 与 SideBySideModernGolden.* 单测中；M2 C-Tier-3 服务接线 12/12 完成（超出原 9 项目标）；`scripts/commercial-smoke.ps1` PASS（MSSQL_E2E LocalDB + GUI_CLIENT_SMOKE 5/5 状态帧 + 30.1% terrain + 原版 BGM + 11842/11844 单元测试）；PlayDH 资源全量审计 433/433=100% OK；BuySyn 货币扣减 + 库存插入、StartSyn 任务接取 + quest_log 写入 已在 modern 单测中验证（含 dealitem/quest 加载路径）| 副作用顺序 / 数值 / DB 的跨实现 diff=0 仍需 modern caster 落地 + legacy SWorking 对照环境 | **M3 modern 闭环完成（含 Ok arm 副作用） + M4 商业 RC 门禁 GREEN (modern 单侧)** |
...
| 玩法/数值 | D1-D6 数据面与 side-effect runtime 已形成广泛单测覆盖；五段核心玩法 modern 侧 capture 全部 byte-for-byte 匹配 modern golden（login/enter_game/attack/shop/quest）；BuySyn 货币扣减 + 库存插入 已在 `MapHandlerTest.BuySynOkArmDeductsMoneyAndInsertsInventory` 中端到端验证（dealitem catalog 命中 → money −qty·price、inventory += qty × ItemBase、回滚路径覆盖 inventory 满 / 资金不足）；StartSyn 任务接取 + quest_log 写入 已在 `MapHandlerTest.StartSynOkArmAddsQuestToPlayerLog` 中端到端验证（quest script 命中 → PlayerRuntime.quest_log 写入 QuestProgress） | 跨实现 side-by-side diff=0 尚未在 legacy 客户端对照环境复现；modern caster 落地后会进一步升级 Skill StartAck + DB 持久化 | **modern 闭环完成（含 Ok 路径），legacy 对照待外部环境** |
...
当前 CMake 发现基线：**11,842 tests**；11,842 项 PASSED，2 项 SKIPPED（真机资源 / E2E），全量 CTest 退出码为 0。该数字用于防止测试静默丢失，不代表 T3 已完成。
...
### M3：T3 五段行为对照 — modern 闭环完成（含 Ok arm 副作用）
...
- 副作用顺序 / 数值 / DB 完整 diff=0 仍需 modern caster 落地 + legacy SWorking 对照环境。

### M4：部署与商业 RC 验收 — 门禁 GREEN (modern 单侧)

- `scripts/commercial-smoke.ps1 -BuildDir modern/build` 全过：MSSQL_E2E LocalDB + GUI_CLIENT_SMOKE 5/5 状态帧 + 30.1% terrain + 原版 BGM + 11842/11844 单元测试（含 2 项 SKIPPED 真机资源 / E2E）。
