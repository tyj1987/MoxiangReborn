...
| 客户端/服务端运行时 | Login/Agent/Map 三进程和五步 E2E；GUI 资源/实体/BGM 可见；MapServer 启动加载真实 ItemList 与 CharacterExpPoint，怪物击杀授予经验、应用活动倍率、发送经验通知并持久化等级/经验，重登录恢复等级 | 任务 EndSyn/奖励仍未接通；完整背包装备持久化和 HUD 仍需完成；跨实现对照仍需 legacy 运行环境 | **登录、进图、战斗、掉落、升级主循环已接通，任务/持久化/UI 继续补齐** |
