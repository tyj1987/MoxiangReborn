...
| GM/运营工具 | GM API 强制认证；玩家列表、状态、角色→账号封禁、审计及聊天日志均接真实 DB；MapServer 世界聊天实时写入 `log_chat`；物品目录直接解析权威 ItemList.bin，支持分页和按 ID 查询；活动与物品发放明确返回 501 | 物品发放需原子库存事务；活动需接权威调度数据面 | **玩家、聊天、物品查询真实闭环；危险写端点安全禁用** |
