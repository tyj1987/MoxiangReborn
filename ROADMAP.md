...
| **M3 进展 (2026-08-10)** | T3 五段 modern 5/5 diff=0；现代金色锁像锁定在 modern/tests/fixtures/sbs_captures_modern/ 与 SideBySideModernGolden.* 单测中；M2 C-Tier-3 服务接线 12/12 完成（超出原 9 项目标）；`scripts/commercial-smoke.ps1` PASS（MSSQL_E2E LocalDB + GUI_CLIENT_SMOKE 5/5 状态帧 + 30.1% terrain + 原版 BGM + 11841/11841 单元测试）；PlayDH 资源全量审计 433/433=100% OK | 副作用顺序 / 数值 / DB 这 3 项需 modern manager (caster / npc_shop / quest_manager) 落地后才能达到真正的玩法 diff=0；跨实现证据仍需 legacy SWorking 运行环境 | **M3 modern 闭环完成 + M4 商业 RC 门禁 GREEN (modern 单侧)** |
...
| 玩法/数值 | D1-D6 数据面与 side-effect runtime 已形成广泛单测覆盖；五段核心玩法 modern 侧 capture 全部 byte-for-byte 匹配 modern golden（login/enter_game/attack/shop/quest） | 跨实现 side-by-side diff=0 尚未在 legacy 客户端对照环境复现；modern manager (caster / npc_shop / quest_manager) 落地后会升级 Nack 为 Ack + DB 持久化 | **modern 闭环完成，legacy 对照待外部环境** |
...
当前 CMake 发现基线：**11,841 tests**；11,841 项基线全量 CTest 退出码为 0。该数字用于防止测试静默丢失，不代表 T3 已完成。
...
- 副作用顺序 / 数值 / DB 完整 diff=0 需要 modern manager (caster / npc_shop / quest_manager) 落地后再升级 Nack → Ack + DB 持久化。

### M4：部署与商业 RC 验收 — 门禁 GREEN (modern 单侧)

- `scripts/commercial-smoke.ps1 -BuildDir modern/build` 全过：MSSQL_E2E LocalDB + GUI_CLIENT_SMOKE 5/5 状态帧 + 30.1% terrain + 原版 BGM + 11841/11841 单元测试。
