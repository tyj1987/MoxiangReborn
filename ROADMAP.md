...
| **M-R4 渲染+UI 视觉 1:1 还原 (2026-08-18, 20 widget class children 装载累计 + DLG 嵌套)** | M-R4.7 commit 1bd3846a — cDialogLoader children 路由扩展到 7 个低频 widget class + 1 DLG 嵌套: cListDialogEx / cMugongDialog / cQuestDialog / cWantedDialog / cJournalDialog / cItemShopGridDialog / cSpin + DLG (cDialog as child, apply_legacy_layout 跟 root 一样). 1:1 with 老版 cScriptManager::GetInfoFromFile eLISTDLGEX / eMUGONGDLG / eQUESTDLG / eWANTEDDLG / eJOURNALDLG / eITEMSHOPGRIDDLG / eSPIN / eDLG 路由. 73/73 PASS (Test 8 扩展到 20 widget class 命中验证, walk 顺序把 cItemShopGridDialog 移到 cIconGridDialog 之前 避免被基类 cast 提前匹配). 实测 M-R4 累计 20 widget class + DLG 嵌套 = 431 children routed. data-only 类型决策 (PAGE 2760 / NPC 141 / MOTION 1) — 跳过不挂 widget (老版 cScriptManager children 路由 default break, 1:1 行为). | **M-R4.8 BLOCKED (2026-08-18)**: 6 stub widget class (WEAREDDLG 4 / PRIVATEWAREHOUSEDLG 5 / MUNPAMARKDLG 1 / SHOPITEMINVENGRID 1 / ANI 0 / SURYUNDLG 1) 调查完成, cDialogLoader_test walk 加 6 dynamic_cast case 后 79/79 PASS 验证 13 命中 1:1. 但 cDialogLoader.cpp 加 6 children 路由触发: ① 4 个 (cWearedExDialog/cMunpaMarkDialog/cPrivateWarehouseDialog/cSuryunDialog) 跟 legacy 1:1 port 完整版 .cpp 实体 ctor 冲突 LNK2005; ② include/mxh/ui/ 是 Windows junction → src/ui/, MSVC #pragma once 把同一文件经两路径视为不同 TU 触发 C2011 cObject/cWindow/cImage 连锁 redefinition. 解决方向: 删 junction 改真目录 (双重维护) / 改用 named include guard 替换 #pragma once (70+ 头) / 改 legacy 1:1 port 4 个 .cpp 移 ctor 到 .hpp inline (改 M-R4 era). 见 memory Moxiang M-R4.8 阻塞 2026-08-18. 待 M-R4 物理 GPU 截屏 SSIM / M-R5 性能 / M-R7 分辨率 | **M-R4.5 + M-R4.6 + M-R4.7 20 widget class children 装载 + DLG 嵌套 + data-only 决策完成, M-R4.8 阻塞** |
