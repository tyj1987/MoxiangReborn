...
> **最近一次状态刷新**：2026-07-31 — Phase C Batch 1.4 完成（cMPNoticeDialog，+11 tests，UI 套件 2194 全过）；Batch 1 进行中（4/10 dialog）。
...
| UI 控件 1:1 port（dialog + subcontrol） | **109/158 hpp（78 missing，Phase C Batch 1.4 完成 4/10）** | 老 client 158 dialog / modern 109 hpp（含 subcontrol+core）；Batch 1 已完成 cNumberPadDialog / cPartyWarDialog / cWantedDialog / cMPNoticeDialog / cMiniNoteDialog / cNoteDialog / cAutoAnswerDlg / cStatusIconDlg / cScreenShotDlg / cOptionDialog / cChannelDialog / cJournalDialog / cHelperSpeechDlg / cCostumeSkinSelectDialog（14 个，193 tests）；剩 77 个老 dialog 待 1:1 port | T3 1:1 行为锁死 |
...
**已完成 4/87**（cNumberPadDialog 2026-07-26 commit `5790051`；cPartyWarDialog 2026-07-31；cWantedDialog 2026-07-31；cMPNoticeDialog 2026-07-31）。
