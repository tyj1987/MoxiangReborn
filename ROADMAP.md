...
| **M3 进展 (2026-08-10)** | T3 五段 modern 5/5 diff=0；现代金色锁像锁定在 modern/tests/fixtures/sbs_captures_modern/ 与 SideBySideModernGolden.* 单测中；M2 C-Tier-3 服务接线 12/12 完成（超出原 9 项目标）；`scripts/commercial-smoke.ps1` PASS（MSSQL_E2E LocalDB + GUI_CLIENT_SMOKE 5/5 状态帧 + 30.1% terrain + 原版 BGM + 11859/11859 单元测试）；PlayDH 资源全量审计 433/433=100% OK；BuySyn 货币扣减 + 库存插入、StartSyn 任务接取 + quest_log 写入 已在 modern 单测中验证（含 dealitem/quest 加载路径）；modern caster data plane (`mxh::server::skill_caster` 纯函数模块) 已独立 + 15 单测覆盖全部 6 个 status 路径 + 1:1 damage 公式 | 副作用顺序 / 数值 / DB 的跨实现 diff=0 仍需 MapHandler 接线 skill_caster + legacy SWorking 对照环境 | **M3 modern 闭环完成（含 caster data plane） + M4 商业 RC 门禁 GREEN (modern 单侧)** |
...
当前 CMake 发现基线：**11,859 tests**；11,859 项 PASSED，全量 CTest 退出码为 0。该数字用于防止测试静默丢失，不代表 T3 已完成。
...
### M3：T3 五段行为对照 — modern 闭环完成（含 Ok arm 副作用 + caster data plane）
...
- 副作用顺序 / 数值 / DB 完整 diff=0 仍需 MapHandler 接线 skill_caster + DB 持久化 + legacy SWorking 对照环境。

### M4：部署与商业 RC 验收 — 门禁 GREEN (modern 单侧)

- `scripts/commercial-smoke.ps1 -BuildDir modern/build` 全过：MSSQL_E2E LocalDB + GUI_CLIENT_SMOKE 5/5 状态帧 + 30.1% terrain + 原版 BGM + 11859/11859 单元测试（含 2 项 SKIPPED 真机资源 / E2E）。
