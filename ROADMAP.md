...
| GM/运营工具 | GM HTTP API 已强制 32+ 字符 Bearer token、常量时间校验、默认仅监听 127.0.0.1，并输出 no-store/nosniff 安全头；64 位 Windows SOCKET 截断已修 | 玩家/物品/聊天/活动端点仍为样例内存数据，需接真实 DB、审计日志和服务器控制面 | **安全边界完成，业务数据面待替换** |

当前 CMake 发现基线：**11,914 tests**；11,914 项 PASSED，全量 CTest 退出码为 0。该数字用于防止测试静默丢失，不代表 T3 已完成。
